...
class CloudEdgeSimulator:
    ...
    def compare_strategies(self, images: list[str | Path]) -> dict[str, Any]:
        ...
        对同一批图像运行三种策略，生成对比报告。
        """
        print(f"[Simulator] 开始对比实验: {len(images)} 张图像")

        strategies = ["edge_only", "cloud_only", "dynamic"]
        results = {}

        for strategy in strategies:
            print(f"  → 运行策略: {strategy}")
            report = self.run_batch(images, strategy=strategy)
        ...
        print("  云边协同策略对比报告")
        print("=" * 80)
        print(f"  {'策略':<15} {'样本数':>8} {'本地数':>8} {'云端数':>8} {'云端比例':>10} {'平均延迟(ms)':>14} {'P90延迟(ms)':>14}")
        print("-" * 80)
        for strategy, rep in results.items():
            if "error" in rep:
                continue
            print(f"  {strategy:<15} {rep['total_images']:>8} {rep['num_local']:>8} {rep['num_cloud']:>8} "
                  f"{rep['cloud_ratio']:>9.1%} {rep['latency_ms']['mean']:>14.1f} {rep['latency_ms']['p90']:>14.1f}")
        print("=" * 80)

        # 路由分布
        dyn = results.get("dynamic", {})
        if "route_distribution" in dyn:
            print(f"\n  [Dynamic] 路由决策分布:")
            for rule, count in dyn["route_distribution"].items():
                print(f"    - {rule}: {count} ({count / dyn['total_images']:.1%})")
