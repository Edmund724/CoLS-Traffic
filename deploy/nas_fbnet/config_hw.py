...
# 搜索阶段总调用次数；控制在 2-3 天（约 40-50 小时）完成
SEARCH_CALLS = 500
# 每个候选架构在搜索阶段训练的最大 epoch 数
# 注：40 epoch 能让分类验证精度更稳定，筛出的 backbone 质量更高
SEARCH_EPOCHS = 40
...
# 可选宽度倍率列表（加入 0.5 以支持 4x 压缩，去掉 1.75/2.0 避免超标）
WIDTH_MULTIPLIER_LIST = [0.5, 0.75, 1.0, 1.25, 1.5]
# 每个 stage 的最小深度
DEPTH_MIN = 2
# 每个 stage 的最大深度
DEPTH_MAX = 4
...
# ── 检测模型参数量约束（以 YOLOv8s 为基准） ──────────────────────────────
BASELINE_PARAMS_M = 11.16656          # YOLOv8s 总参数量 (M)
TARGET_COMPRESSION_RATIO = 4.0        # 目标压缩比
MAX_DETECTOR_PARAMS_M = BASELINE_PARAMS_M / TARGET_COMPRESSION_RATIO  # ≈ 2.79 M

# 检测模型 Neck 通道配置（配合小 backbone 实现 4x 压缩）
DET_NECK_P3 = 96
DET_NECK_P4 = 192
DET_NECK_P5 = 192
DET_NUM_CLASSES = 10                  # DAIR-V2X 类别数
