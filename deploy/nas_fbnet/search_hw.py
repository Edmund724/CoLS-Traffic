...
def _estimate_detector_params(arch_config: dict) -> float:
    """根据 NAS backbone arch_config 估算完整检测模型总参数量 (M)。"""
    import sys
    ...
    backbone = NASBackboneFeat.from_arch(arch)
    model = NASYOLOv8(
        backbone,
        nc=DET_NUM_CLASSES,
        p3_ch=DET_NECK_P3,
...
def _make_objective_hw(
    ...
    def _log_row2(
        ...
        *,
        ...
        row = {
            ...
            "detector_params_m": f"{detector_params_m:.4f}" if detector_params_m is not None else "",
    ...
    def objective(params):
        ...
        # ── 参数量约束：估算完整检测模型参数量 ───────────────────────────
        det_params = _estimate_detector_params(arch_config)
        print(f"  检测模型估算参数量: {det_params:.3f}M (上限 {MAX_DETECTOR_PARAMS_M:.3f}M)")
        if det_params > MAX_DETECTOR_PARAMS_M:
            print(f"  超过参数量上限 {MAX_DETECTOR_PARAMS_M:.3f}M，跳过训练")
            evaluated_cache[key] = HW_SKIP_LOSS
            evals.append({"params": list(key), "result": HW_SKIP_LOSS})
            _save_hw_cache(cache_path, evals)
            trial_elapsed = time.perf_counter() - trial_start_time
            ...
            _log_row2(
                ...
                hw=hw_pred,
                combined=None,
                detector_params_m=det_params,
                hw_skip_reason="param_budget_exceeded",
                trial_elapsed_sec=trial_elapsed,
                time_taken_sec=trial_elapsed,
                search_elapsed_sec=search_elapsed,
                run_elapsed_sec=run_elapsed,
...
def run_search_hw():
    ...
    print(f"  参数量约束: 以 YOLOv8s ({BASELINE_PARAMS_M:.2f}M) 为基准，压缩比 {TARGET_COMPRESSION_RATIO}x")
    print(f"  检测模型上限: {MAX_DETECTOR_PARAMS_M:.3f}M  (Neck={DET_NECK_P3}/{DET_NECK_P4}/{DET_NECK_P5})")
