...
class NASYOLOv8(nn.Module):
    ...
    def forward(self, x):
        ...
        # Top-down
        p5 = self.sppf(c5)
        p4 = self.c2f_p4(torch.cat([self.up1(p5), c4], dim=1))
        p3 = self.c2f_p3(torch.cat([self.up2(p4), c3], dim=1))
    ...
    def load(cls, detector_ckpt: str):
        ...
        ck = torch.load(detector_ckpt, map_location="cpu")
        ...
        if backbone_spec is None:
            raise KeyError("backbone_spec not found in checkpoint.")
...
if __name__ == "__main__":
    ...
    print(f"  -> {preds.shape[1]-4} classes, {preds.shape[2]} anchor points across 3 scales")
